...
const Packages = () => {
  ...
  const saveEdit = async () => {
    ...
    try {
      setSaving(true);
      const token = localStorage.getItem("token");
      const { _id, name, description, price, image, isAvailable } = editingPackage;
      await axios.put(
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true);
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:5000/api/packages/${confirmDeleteId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <Layout>
      <div className="bg-[#30343c] min-h-screen w-full text-white p-8">
        ...
        {/* Edit Package Modal */}
        {editingPackage && (
          <Dialog open={!!editingPackage} onOpenChange={(open) => !open && closeEdit()}>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  {editingPackage.image && (
                    <div className="mb-2">
                      <img src={editingPackage.image} alt="preview" className="h-20 w-20 object-cover rounded border border-gray-700" />
                    </div>
                  )}
                  <input
                    type="file"
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="pkg-available" className="text-sm text-gray-300">Available</label>
                </div>
              </div>
              <div className="mt-4 flex justify-end gap-3">
                <button onClick={closeEdit} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded border border-gray-600">Cancel</button>
                <button onClick={saveEdit} disabled={saving} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded border border-blue-500 disabled:opacity-60">{saving ? "Saving..." : "Save"}</button>
              </div>
            </DialogContent>
          </Dialog>
        )}
        ...
        <Dialog open={!!confirmDeleteId} onOpenChange={(open) => !open && cancelDelete()}>
          <DialogContent className="max-w-sm bg-gray-900 text-white border border-gray-700">
            ...
            <div className="text-gray-300 mb-4">Are you sure you want to delete this package? This action cannot be undone.</div>
            <div className="flex justify-end gap-3">
              <button onClick={cancelDelete} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded border border-gray-600">Cancel</button>
              <button onClick={confirmDelete} disabled={deleting} className="px-4 py-2 bg-red-600 hover:bg-red-700 rounded border border-red-500 disabled:opacity-60">{deleting ? "Deleting..." : "Delete"}</button>
            </div>
          </DialogContent>
        </Dialog>
